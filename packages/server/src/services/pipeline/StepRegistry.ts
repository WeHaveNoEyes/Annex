...
// biome-ignore lint/complexity/noStaticOnlyClass: Registry pattern provides namespacing for step management
export class StepRegistry {
  ...
  static register(type: StepType, stepClass: StepConstructor): void {
    StepRegistry.steps.set(type, stepClass);
  }
  ...
  static create(type: StepType): BaseStep {
    const StepClass = StepRegistry.steps.get(type);
    if (!StepClass) {
  ...
  static has(type: StepType): boolean {
    return StepRegistry.steps.has(type);
  }

  // Get all registered step types
  static getRegisteredTypes(): StepType[] {
    return Array.from(StepRegistry.steps.keys());
  }
